...
class MaxAccelerationRatioException(Exception):
    ...
    def __init__(self, given_ratio: float, message=f"Max acceleration ratio is not ({config.get('min_max_acceleration_ratio')}, {config.get('max_acceleraton_ratio')}) km/h range. "):
